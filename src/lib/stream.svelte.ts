...
/**
 * Merge queued notes into a loaded stream.
 *
 * Every note is a peer now — a thought filed into `#dental` is a note in the
 * stream like any other, not something folded underneath an earlier one — so
 * this is just a prepend. A note that has already come back from the server is
 * dropped from the pending side by `client_id`, so the hand-off between the
 * two is invisible rather than a flicker of duplicates.
 */
export function withPending(notes: Note[]): Note[] {
	if (pending.items.length === 0) return notes;

	const known = new Set<string>();
	for (const note of notes) if (note.client_id) known.add(note.client_id);

	const queued = pending.items.filter((n) => !(n.client_id && known.has(n.client_id)));
	return queued.length === 0 ? notes : [...queued, ...notes];
}
