	...
	>
		{#if isLink}
			...
			>
				...
				<span class="min-w-0 flex-1">
					{#if heading}
						<span class="block truncate text-[0.76rem] font-semibold" style="color: var(--color-ink-2);">
							{heading}
						</span>
					{/if}
					{#if note.source_description}
						<span class="mt-1 block truncate text-[0.72rem] leading-[1.35]" style="color: var(--color-ink-faint);">
							{note.source_description}
						</span>
					{/if}
				</span>
				<span class="shrink-0 text-[0.58rem] font-semibold tracking-[0.1em] uppercase" style="color: var(--color-ink-faint);">
		...
		{:else if hasEmbeddedImage}
			<div class="flex items-start gap-4">
				...
				/>
				{#if textOnly.trim()}
					<p class="line-clamp-3 min-w-0 flex-1 font-serif text-[1.12rem] leading-[1.5] tracking-[-0.01em] whitespace-pre-wrap">
						{excerpt(textOnly, 200)}
					</p>
				{/if}
			</div>
		{:else}
			<p class="line-clamp-6 font-serif text-[1.12rem] leading-[1.5] tracking-[-0.01em] whitespace-pre-wrap">
				{body}
			</p>
		{/if}
	</a>

	<div class="mt-3 flex items-center gap-2.5 text-[0.72rem] font-medium tracking-[-0.01em]" style="color: var(--color-ink-faint);">
		{#each note.tags as tag (tag.id)}
			<TagChip {tag} />
		{/each}
		<span class="flex-1"></span>
		{#if isLink && note.source_url}
			<span class="max-w-[9rem] truncate" style="color: var(--color-ink-muted);">{hostname(note.source_url)}</span>
		{/if}
		{#if showTime}
		...
		{#if !queued}
			...
			>
				<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.7" stroke-linecap="round">
		...
		{#if expanded}
			<div class="mt-4 space-y-5">
				{#each thoughts as thought (thought.id)}
					<Thought note={thought} href={isPending(thought) ? null : `/note/${thought.id}`} max={240} />
				{/each}
			</div>
		{/if}
	{/if}
